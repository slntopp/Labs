...
class VectException : std::exception {
    private:
        std::string whatStr;
    public:
        VectException(std::string &&whatStr) noexcept : whatStr(std::move(whatStr)) { };
        VectException(const std::string &whatStr) noexcept : whatStr(whatStr) { };
        ~VectException() noexcept = default;
...
class Vector {
    ...
    public:
        ...
        double operator[] (int index) const {
            if(index >= amount) throw VectException("_INDEX_ERROR_501");
            return val[index];
        }
};
