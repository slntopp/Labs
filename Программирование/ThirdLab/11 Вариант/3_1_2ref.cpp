...
//и принимающую три переменные типов double, int и ссылку на double. Функция будет содержать в себе
//"тело" алгоритма
...
//текущее Х меньше либо равно Х-конечному
		...
		body(x, n, result); //Вызов функции body, в функция отправляется реальная переменная, так как функция принимает
