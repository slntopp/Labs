...
int main() { // функция main
	...
	double x0 = 0, dx = 0.4, xn = 4, xi = x0, y, p=1; // обьявление нужных нам переменных вещественного типа double
	...
	for (i = 0; xi < xn; i++) {// начало  первого  цикла for
		...
		y = n*p; // подробнее смотри сопутствующую справку, страница 21

		for (k = n-1; k >= 0; k--) { // начало второго цикла for
			if (k % 2 == 0) {// при четных степенях по условию
				y = y*xi + k;
			}
			else {// при нечетных степенях по условию
				y = y*xi - k;
			}
